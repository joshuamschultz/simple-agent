...
And one thing that is deliberately NOT growth: run_python runs a block of
code in that same sandbox, with every grown tool already in scope, and keeps
nothing. It is the scratch path — the loop, the filter, the count that would
be a bad permanent tool. It is also how work stops costing context: a tool
call puts its whole result in front of the model forever, where code puts
only what it prints. Anything worth keeping gets grown afterwards.

Two ways to face something bigger than the context window, and the choice is
the whole trick: run_python when a filter or a count answers it, survey when
it takes judgment to find. They hand off to each other — output too large to
read is saved to a file and offered back as a path to survey.
...
# agent from then on. So a key can go missing — a prompt it edited badly, a
# hand-edit that dropped one — and a missing prompt is not a crash, it is an
# agent quietly stopping being told something. Check on load, where it can
# still be fixed.
REQUIRED_PROMPTS = ("identity", "draft", "survey", "merge", "compact", "carried", "review")
...
COMPACT_RESULT_CHARS = 400 # of a tool result, how much the compactor gets to read
CODE_OUTPUT_CHARS = 4000   # of what run_python prints, what may enter the window.
                           # Past this the whole output goes to a file and comes
                           # back as a path — the point of running code is to
                           # keep the data out here, not to move the flood.
...
for _n, _f in _ns.items():
    if callable(_f): setattr(self, _n, _t.MethodType(_f, self))
_RESULT = ""
{body}
print("__RESULT__" + json.dumps({{"result": str(_RESULT)}}))
'''

# The two things a body can be. Everything else about the sandbox — the shim,
# the grown tools, the cwd, the timeout, the redaction — is identical, which
# is the reason run_python needed no second sandbox.
_CALL_ONE = "_RESULT = getattr(self, {fn_name!r})(**json.loads(sys.argv[1]))"
...
class SelfBuildingAgent:
    _META_ROOT = ("grow_tool", "read_tool", "grow_skill", "read_skill", "forget_skill",
                   ...
                   "run_python", "survey", "ask_user")
    _META_CHILD = ("grow_tool", "read_tool", "grow_skill", "read_skill",
                    "update_identity", "spawn_agent", "run_python", "survey",
                    "ask_user")
    ...
    def __init__(self, client=None, workspace: str = "agent_workspace",
        ...
        self.summary = ""                      # and what has fallen off the front of it
    ...
    def _run_sandboxed(self, fn_name: str, kwargs: dict) -> str:
        ...
        return self._sandbox(_CALL_ONE.format(fn_name=fn_name), json.dumps(kwargs))

    def _sandbox(self, body: str, argv: str = "{}") -> str:
        """Run one body of code in a subprocess and bring back one string.

        The single door into execution. Every grown tool's source is defined
        first and bound to `self`, so a body reads exactly like a tool body:
        self.<tool>(...), self._raw_read(), self._secret(...). What comes back
        is whatever the body printed plus whatever it left in _RESULT, with
        every stored secret redacted out of it.
        """
        runner = _RUNNER.format(
            manifest_code="\n\n".join(self._code(e) for e in self.manifest.values()),
            body=body)
    ...
    def _child(self, identity: str, workspace: str | None = None,
        ...
        # Your words, not stock ones. If you sharpened how you draft a tool or
        # read a store, a child doing that work for you drafts and reads the
        # sharpened way. Only who it is differs, and that is set next.
        child.prompts = dict(self.prompts)
    ...
    def run_python(self, code: str) -> str:
        """Run Python now, in the sandbox, with everything you have grown in
        scope: self.<tool>(...), self._raw_read(), self._raw_write(...),
        self._secret(...), open(), any import. Nothing is registered or kept,
        and each run is a fresh process — put what you need later in the store
        or a file.

        ONLY what you print comes back, so print the answer, not the data.
        That is the whole point: a tool call puts its entire result in front of
        you for the rest of the session, where code can read a hundred thousand
        records and hand you one number. Use it to loop over a tool, filter,
        count, join two tools, or look before deciding what to build. Writing
        the same block twice is the signal to grow_tool it.

        survey reads when the answer takes judgment; this one computes."""
        out = self._sandbox(f"{code}\n")
        if out.startswith(FAILED):
            return out
        if not out.strip():
            return ("Ran without error and printed nothing. Nothing comes back but what "
                    "you print — add a print() for the part you wanted to see.")
        return self._clip(out, CODE_OUTPUT_CHARS, "printed")

    def _clip(self, text: str, limit: int, verb: str) -> str:
        """Keep an oversized result out of the window without losing any of it.

        Anything past the limit is written to a file in the workspace and the
        head comes back with the path. Nothing is silently dropped — a
        truncated answer that does not say it was truncated is the one failure
        mode worse than no answer. The path is the handoff between the two
        ways of reading something too long: narrow it with more code, or hand
        it to survey when it needs judgment.
        """
        if len(text) <= limit:
            return text
        path = os.path.join(self.workspace, f"out_{uuid.uuid4().hex[:8]}.txt")
        try:
            with open(path, "w") as f:
                f.write(text)
        except OSError as e:
            return (f"{text[:limit]}\n\n[{len(text):,} characters {verb}. The rest could "
                    f"not be saved either ({e}), so it is gone. Print less.]")
        return (f"{text[:limit]}\n\n[{len(text):,} characters {verb} — far too many to read "
                f"here, and this is the first {limit:,}. All of it is saved at {path}. "
                f"Run more code against that file to narrow it down, or survey it if "
                f"finding the answer takes judgment.]")
    ...
    def survey(self, question: str, source: str = "memory") -> str:
        ...
        This is the expensive read: a whole agent per part. Use it when finding
        the answer takes judgment — what is here, what is odd, what is missing.
        When a filter, a count or a lookup by key would settle it, run_python
    ...
    def read_prompt(self, name: str) -> str:
        ...
        review your own work), 'compact' (how the conversation is boiled down
        when it outgrows the window), 'carried' (how what is left of it is put
        back in front of you). Read before rewriting."""
    ...
    def update_prompt(self, name: str, text: str) -> str:
        ...
        if name not in self.prompts:
            return f"{FAILED}no prompt named '{name}'. You have: {list(self.prompts)}"
        # Measured against the version being replaced, not a stock copy — there
        # isn't one. Every write goes through here, so a placeholder present at
        # install is still present now, however many rewrites deep.
        missing = [ph for ph in _re.findall(r"\{(\w+)\}", self.prompts[name])
                   if "{" + ph + "}" not in text]
        if missing:
    ...
    def _build_system(self) -> str:
        ...
        if self.summary:
            # Rides in the prompt rather than in the message list, so the next
            # trim cannot throw away the record of the last one. The wording
            # around it is a prompt like any other: it lives in state, and the
            # agent can rewrite it. Only the note itself is filled in here.
            parts.append(self.prompts["carried"].format(summary=self.summary))
        return "\n\n".join(parts)

    # ---------------- narration ----------------

    def _emit(self, phase: str, tool: str, args: dict | None = None,
              result: str = "", fresh: bool = False, meta: bool | None = None) -> None:
        ...
        try:
            self.on_event({"phase": phase, "tool": tool, "args": args or {},
                           ...
                           "meta": (tool in self._meta_names()) if meta is None else meta,
            ...
            for call in reply["tool_calls"]:
                ...
                try:
                    if name in meta:
                        ...
                        result = self._run_sandboxed(name, args)
                    else:
    ...
    def _trim_conversation(self) -> None:
        """Hold the conversation to CONTEXT_MESSAGES by COMPACTING what falls
        off the front of it, never by simply deleting it.

        Whole exchanges leave together — cutting mid-exchange would orphan a
        tool call — and what leaves is folded into a running note that rides
        in the system prompt from then on. Deleting outright is silent
        amnesia: the model goes on answering with no way to know that what it
        needed was thrown away two turns ago, and the user finds out first.
        """
        dropped: list[dict] = []
    ...
    def _compact(self, dropped: list[dict]) -> None:
        """Fold the exchanges that just left into the running summary.

        One model call, and it must never cost a turn: those messages are
        already out of the window by the time we get here, so a compaction
        that fails leaves a visible hole in the note rather than raising.
        Saying the hole is there beats pretending nothing was lost.
        """
        transcript = self._render(dropped)
        if not transcript.strip():
            return
        try:
            reply = self._complete(
                self.prompts["compact"].format(summary=self.summary or "(nothing yet)"),
                [{"role": "user", "content": transcript}])
            note = (reply["content"] or "").strip()
        except Exception:
            note = ""
        self.summary = note or (
            f"{self.summary}\n[{len(dropped)} earlier messages left the window and could "
            f"not be summarized — whatever was in them is gone]").strip()
        self._emit("end", "compact", {"messages": len(dropped), "note": self.summary},
                   f"summary now {len(self.summary)} chars", meta=True)

    @staticmethod
    def _render(messages: list[dict]) -> str:
        """A stretch of conversation as plain text, for a reader who gets the
        text and nothing else. Tool results are clipped — the compactor needs
        to see what a call came back with, not read all of it again."""
        out = []
        for m in messages:
            who, content = m["role"].upper(), m["content"]
            ...
            for block in content:
                kind = block.get("type")
                if kind == "text":
                    out.append(f"{who}: {block['text']}")
                elif kind == "tool_use":
                    out.append(f"  tool {block['name']}"
                               f"({json.dumps(block.get('arguments', {}))[:200]})")
                elif kind == "tool_result":
                    out.append(f"  -> {str(block.get('content', ''))[:COMPACT_RESULT_CHARS]}")
        return "\n".join(out)
    ...
    def load(self, path: str):
        ...
        # This file is the only place the words live. setup.sh put them here on
        # install; every edit since is the agent's own. Nothing overlays a
        # default on top, because there is no default to overlay.
        ...
        # The conversation is deliberately NOT restored, and neither is the
        # compacted note of it — that note IS the conversation, in shorter
        # form. A restart is a clean slate for the thread; what persists is
        # what was learned — tools, skills, memory, identity.
        self.messages = []
        self.summary = ""
...
_KIND = {"grow_tool": "tool", "read_tool": "tool",
         ...
         "survey": "store", "compact": "prompt", "run_python": "tool",
...
def _columns(tool: str, args: dict, result: str) -> tuple[str, str]:
    ...
    if tool == "run_python":
        # The code is the readable part, and its first line usually says what
        # the block is for. The return is whatever it chose to print.
        return _one_line(args.get("code"), 22), _one_line(result, 44)
    if tool == "compact":
        return f"{args.get('messages')} messages", _one_line(args.get("note"), 44)
    if tool == "survey":
...
def _main():
    ...
    # The state file is not optional any more: it holds the words, so without it
