...
# Every prompt this file fills in. prompts.py is regenerated by setup.sh and
# owned by you from then on, so a key can go missing across versions — and a
# missing prompt is not a crash, it is an agent that quietly stops being told
# something. Say it once, at startup, where it can still be fixed.
_MISSING = [k for k in ("identity", "draft", "survey", "merge", "compact", "carried", "review")
            if k not in DEFAULT_PROMPTS]
if _MISSING:
    sys.exit(f"prompts.py is missing {_MISSING}. Re-run ./setup.sh to reseed it — your own "
             f"edits live in agent_state.json and win over the seed either way.")
